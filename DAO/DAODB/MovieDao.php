<?php
    ...
    class MovieDao extends Singleton implements IDao
    {
        ...
        {
            $sql = "SELECT m.id_movie,m.title,m.lenght,m.language,m.image,m.overview,m.background FROM movies m INNER JOIN shows s ON m.id_movie = s.id_movie WHERE s.show_date >= curdate() GROUP BY m.id_movie";
            try {
                $this->connection = Connection::getInstance();
                $resultSet = $this->connection->execute($sql);
            } catch (PDOException $e) {
        ...
        {
            $sql = "SELECT m.id_movie,m.title,m.lenght,m.language,m.image,m.overview,m.background FROM movies m INNER JOIN shows s ON m.id_movie = s.id_movie INNER JOIN genre_x_movie gm ON M.id_movie = GM.id_movie WHERE GM.id_genre = :id_genre AND s.show_date >= curdate() GROUP BY M.id_movie";
        ...
        {
            $sql = "SELECT m.id_movie,m.title,m.lenght,m.language,m.image,m.overview,m.background FROM movies m INNER JOIN shows s ON m.id_movie = s.id_movie WHERE date_format(s.show_date,'%Y-%m-%d') = :dateForSearch GROUP BY m.id_movie";
